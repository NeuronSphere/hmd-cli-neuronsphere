...
Extend Mode Creates The Deployment GUI Database
    [Tags]    integration    extend-mode    phase2    gui
    [Documentation]    The GUI keeps Django models (permissions, ChangeSet drafts,
    ...    audit log, MCP API keys) in its own Postgres database. It is a
    ...    control-plane container, so like ms-naming and ms-deployment its
    ...    database is created directly by ensure_core_databases_direct rather
    ...    than through an environment's ms-dbaccount.
    Database Should Exist    hmd_db    deployment_gui

Extend Mode Serves The Deployment GUI Health Endpoint
    [Tags]    integration    extend-mode    phase2    gui
    [Documentation]    hmd_proxy listens on the GUI's host port and proxies
    ...    straight to the container -- no Ingress, no /etc/hosts entry. The port
    ...    is resolved, not hardcoded, so HMD_LOCAL_GUI_HOST_PORT is honoured. The
    ...    container retries `manage.py migrate` until its database exists, which
    ...    is what the timeout covers.
    ${gui}=    Get Deployment GUI URL
...
Extend Mode Does Not Deploy The Deployment GUI Through The DAG
    [Tags]    integration    extend-mode    phase2    gui
    [Documentation]    The BOM entries were removed when the GUI moved to compose;
    ...    if they came back, every `up` would again wait on a Helm release into
    ...    k3s for a UI that is already running.
    ${bom}=    Get Deployment BOM    local
    ${names}=    Instance Names From    ${bom}
    Should Not Contain    ${names}    deployment-gui
    Should Not Contain    ${names}    deployment-gui-db-account
