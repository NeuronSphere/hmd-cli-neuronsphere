...
- fix: a first run's interfaces are reachable by name. The routing that reads
  what a deploy created -- the Ingress hosts, the ALB wildcard paths, and
  Trino's host port -- ran only *before* the deploy, so on a first run it found
  nothing and the summary pointed at hmd-cli-helm's hardcoded
  `<instance>.local.neuronsphere.io`, which resolves nowhere; Trino answered 404
  on every path and nothing listened on its host port. All three healed on a
  second `nsctl env start` that nobody knew to run. The steps are now one
  function called from both sides of the deploy, with a test that fails if the
  post-deploy call is dropped.
- feat: before deploying onto a new cluster, check inside the cluster's own
  network namespace that bridged frames reach netfilter, and say what will break
  if they do not. The namespace matters: the same sysctl set on the engine host
  leaves a cluster working, and set inside the container breaks every ClusterIP.
- feat: when a deploy fails, print what the cluster knows -- its recent warning
  events and any ExternalSecret that is not ready. A helm release rolled back on
  a deadline reports only that it ran out of time, and `--atomic` has already
  deleted the pods that would have shown why, so it is read at the moment of
  failure or not at all.
- fix: `nsctl doctor` and `nsctl dns status` no longer blame the control plane
  when the real cause is Colima's default `portForwarder: ssh`, which carries
  TCP only and so cannot reach the resolver's UDP port however healthy it is
  inside the VM. Both now name `portForwarder: grpc` and the restart.
- fix: the `host names` warning no longer offers `nsctl dns install`, which
  cannot fix it. Those are bare single-label names and no suffix-scoped resolver
  can claim them, so someone who followed the advice found the warning still
  there with nothing to say why. Only the /etc/hosts line is offered now.
- fix: `engine capacity` says when a machine clears the floor but has no room
  for a full stack, instead of reporting a flat ok.

- feat: load `br_netfilter` on the engine's kernel before building a cluster,
  and report it as a `bridge netfilter` row in `nsctl doctor`. Without the
  module, bridged frames bypass netfilter: every pod on the single-node cluster
  shares one bridge, so a reply to a ClusterIP comes back carrying the backend
  pod's own address and is dropped. Cluster DNS fails first, then every Service
  call -- while the node reports `Ready` throughout, so one kernel property
  arrives as three unrelated-looking bugs: ExternalSecrets that never sync,
  ingress-backed interfaces answering 503, and LoadBalancer ports refusing
  connections. Docker Desktop's kernel loads it already; a freshly created
  Colima VM does not, and k3s cannot load it itself because Floci gives its
  container no `/lib/modules` and offers no knob to add one. The load is
  announced, skippable with `HMD_LOCAL_NS_NO_KERNEL_SETUP`, silent when the
  module is already loaded, and never fails a start -- a kernel that cannot be
  prepared falls through to the doctor row and to the cluster image's own
  refusal, both of which name the one command that fixes it. See NERD028.
