"""
Telemetry service plugin for local NeuronSphere.
...
_PLUGIN_NAME = "telemetry"
_dirname = Path(os.path.dirname(__file__))
_services_dir = _dirname / ".." / "services"


def enabled(config_overrides: Dict[str, bool] = {}) -> bool:
    """Check if telemetry plugin is enabled."""
    config = load_nsplugin_config(_PLUGIN_NAME)
    if config:
        env_var = config.get(
            "env_var_override", "HMD_LOCAL_NEURONSPHERE_ENABLE_TELEMETRY"
        )
    else:
        env_var = "HMD_LOCAL_NEURONSPHERE_ENABLE_TELEMETRY"
...
def get_resources() -> Dict[str, Any]:
    ...
    return {"endpoints": ["prometheus:localhost:9090"]}
...
def _prepare_hmd_home_legacy(hmd_home: Path, configs: Dict[str, bool] = {}) -> None:
    ...
    shutil.copy2(
        _services_dir / "telemetry" / "otel-collector-config.yaml",
        hmd_home / ".cache" / "telemetry" / "otel-collector-config.yaml",
    )

    shutil.copy2(
        _services_dir / "telemetry" / "prometheus.yaml",
        hmd_home / ".cache" / "telemetry" / "prometheus.yaml",
    )
...
def render_compose_yaml(
    ...
    if not compose_path:
        ...
        compose_path = get_bundled_compose_path("docker-compose.telemetry.yml")
