"""
Trino service plugin for local NeuronSphere.
...
def enabled(config_overrides: Dict[str, bool] = {}) -> bool:
    """Check if trino plugin is enabled."""
    config = load_nsplugin_config(_PLUGIN_NAME)
    if config:
        env_var = config.get("env_var_override", "HMD_LOCAL_NEURONSPHERE_ENABLE_TRINO")
    else:
        env_var = "HMD_LOCAL_NEURONSPHERE_ENABLE_TRINO"
...
def render_compose_yaml(
    ...
    if not compose_path:
        ...
        compose_path = get_bundled_compose_path("docker-compose.trino.yml")
