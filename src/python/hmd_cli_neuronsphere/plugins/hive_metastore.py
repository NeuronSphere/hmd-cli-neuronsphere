"""
Hive Metastore service plugin for local NeuronSphere.
...
def enabled(config_overrides: Dict[str, bool] = {}) -> bool:
    ...
    if config:
        env_var = config.get(
            "env_var_override", "HMD_LOCAL_NEURONSPHERE_ENABLE_HIVE_METASTORE"
        )
    else:
        env_var = "HMD_LOCAL_NEURONSPHERE_ENABLE_HIVE_METASTORE"
...
def get_resources() -> Dict[str, Any]:
    ...
    return {
        "services": [{"name": "metastore", "url": "thrift://metastore:9083"}],
        "databases": [
            {"username": "metastore", "password": "metastore", "database": "metastore"}
        ],
    }
